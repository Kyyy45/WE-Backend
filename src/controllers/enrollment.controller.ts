...
export const updateProgress = async (
  ...
  if (
    !courseDoc ||
    !courseDoc.teacher ||
    courseDoc.teacher._id.toString() !== req.user?.id
  ) {
  ...
  if (
    ...
    return res
      .status(400)
      .json({ message: "Progress cannot be reduced after completion" });
  }
  ...
  if (progress === 100) {
    try {
      ...
      if (!existing) {
        const prefix = "WPE";
        const courseCode = courseDoc.title
          .split(" ")[0]
          .toUpperCase()
          .substring(0, 3);
        const count = await Certificate.countDocuments();
        const certId = `${prefix}-${courseCode}-${(count + 1)
          .toString()
          .padStart(4, "0")}`;
        ...
        logger.info(
          `🎓 Certificate generated for ${courseDoc.title} (${certId})`
        );
      } else {
        logger.info(
