...
import { getCache, setCache } from "../utils/dashboardCache"; // digunakan untuk sinkronisasi dashboard
...
export const createTransaction = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const { courseId, paymentMethod = "qris" } = req.body;
    ...
    const transaction = await Transaction.create({
      ...
      qrCodeUrl: data.actions?.find((a: any) => a.name === "generate-qr-code")?.url || null,
      status: "pending",
...
export const handleCallback = async (req: Request, res: Response) => {
  try {
    ...
    if (["settlement", "capture"].includes(transaction_status)) newStatus = "success";
...
export const getMyTransactions = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    logger.error("getMyTransactions error: " + err.message);
    return res.status(500).json({ message: "Server error" });
  }
};
