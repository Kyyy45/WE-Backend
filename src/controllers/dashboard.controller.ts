...
export const getDashboard = async (
  ...
  if (user.role === "admin") {
    ...
    const myCourses = await Course.find({ teacher: user.id })
      .select("_id title isPublished")
      .lean();
    const courseIds = myCourses.map((c) => c._id);
    const myEnrollCount = await Enrollment.countDocuments({
      course: { $in: courseIds },
    });
    ...
    const avgProgress =
      myEnrollments.reduce((acc, e: any) => acc + (e.progress || 0), 0) /
      (totalCoursesTaken || 1);

    const certificates = await Certificate.find({ student: user.id })
      .select("certificateId courseName certificateLink createdAt")
      .lean();
