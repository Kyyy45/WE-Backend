...
export const uploadThumbnailMiddleware = multer({ storage }).single("thumbnail");
...
export const uploadCourseThumbnail = async (req: Request, res: Response) => {
  try {
    ...
    logger.error("uploadCourseThumbnail error: " + err.message);
...
export const deleteCourseThumbnail = async (req: Request, res: Response) => {
  try {
    ...
    logger.error("deleteCourseThumbnail error: " + err.message);
    res.status(500).json({ message: "Server error" });
  }
};
